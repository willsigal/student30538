...
In virtually every example, we will associate **attribute** data with vectors -- these are what might appear in a table with vector data, bu
...
When the object is a `Point`, the `WKT` is very similar to the coordinates. As the objects get more complicated, the formatting of `WKT` starts to deviate from coordinates. 
