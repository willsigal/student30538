...
- Note: you can set it up manually, or you can load a basic Shiny template from command line like we did last class
...
## Run the app
- We're not done developing the app, but it's good practice to try running it every time we add something new
- In Terminal:
...
- Question: given how we've defined the dropdown menu on the **UI-side** (below), do you remember how we reference the state the user selects on the **server-side**?
...
- start with the easier step of adding radio buttons on the UI side
